...
Message: ${message}`;
  ...
  (function initReviewsCarousel(){
    ...
    let current = 0;
    let autoplay = true;
    const INTERVAL = Math.floor(Math.random() * 1000) + 3000; // random 3-4 seconds (3000-4000ms)
    let timer = null;
    const reduced = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    ...
    // show a slide
    function show(i){
      // Ensure only the target slide is visible and accessible
    ...
    function next(){ 
      // Random next review instead of sequential
      const availableIndices = slides.map((_, idx) => idx).filter(idx => idx !== current);
      const randomIndex = availableIndices[Math.floor(Math.random() * availableIndices.length)];
      goTo(randomIndex); 
    }
    function prev(){ goTo(current-1); }

    // Autoplay handling with random intervals
    function startAutoplay(){ 
      if (reduced) return; 
      stopAutoplay(); 
      const randomInterval = Math.floor(Math.random() * 1000) + 3000; // 3-4 seconds
      timer = setInterval(next, randomInterval); 
    }
    function stopAutoplay(){ if (timer) { clearInterval(timer); timer = null; } }
